...
Submitted: ${new Date().toLocaleString('en-US', { timeZone: 'America/Chicago' })}
  ...
  // GET /api/forum/needs - List all need posts with filters
  app.get("/api/forum/needs", async (req, res) => {
    try {
      const { needType, locationState, locationCity, urgency, status, userId, search } = req.query;
      const posts = await storage.getForumNeedPosts({
        needType: needType as string,
        locationState: locationState as string,
  ...
  // GET /api/forum/needs/:id - Get single need post
  app.get("/api/forum/needs/:id", async (req, res) => {
    try {
      const post = await storage.getForumNeedPost(req.params.id);
      if (!post) {
        return res.status(404).json({ error: "Need post not found" });
      }
      
      // Increment view count
      await storage.incrementNeedPostViews(req.params.id);
  ...
  // GET /api/forum/needs/slug/:slug - Get need post by slug
  app.get("/api/forum/needs/slug/:slug", async (req, res) => {
    try {
      const post = await storage.getForumNeedPostBySlug(req.params.slug);
      if (!post) {
        return res.status(404).json({ error: "Need post not found" });
      }
      
      // Increment view count
      await storage.incrementNeedPostViews(post.id);
  ...
  // POST /api/forum/needs - Create new need post (authenticated)
  app.post("/api/forum/needs", isAuthenticated, async (req: any, res) => {
    try {
      const currentUser = await getCurrentUser(req);
      if (!currentUser) {
        return res.status(401).json({ error: "Unauthorized" });
      ...
      const validated = insertForumNeedPostSchema.parse({
        ...req.body,
        userId: currentUser.userId,
        slug,
      });
      
      const post = await storage.createForumNeedPost(validated);
      res.json(post);
    } catch (error: any) {
      res.status(400).json({ error: error.message });
    }
  ...
  // PATCH /api/forum/needs/:id - Update need post
  app.patch("/api/forum/needs/:id", isAuthenticated, async (req: any, res) => {
    try {
      const currentUser = await getCurrentUser(req);
      if (!currentUser) {
        return res.status(401).json({ error: "Unauthorized" });
      ...
      const updated = await storage.updateForumNeedPost(req.params.id, req.body);
      res.json(updated);
    } catch (error: any) {
      res.status(400).json({ error: error.message });
    }
  ...
  // DELETE /api/forum/needs/:id - Delete need post
  app.delete("/api/forum/needs/:id", isAuthenticated, async (req: any, res) => {
    try {
      const currentUser = await getCurrentUser(req);
      if (!currentUser) {
        return res.status(401).json({ error: "Unauthorized" });
      ...
      await storage.deleteForumNeedPost(req.params.id);
      res.json({ message: "Need post deleted successfully" });
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  });
